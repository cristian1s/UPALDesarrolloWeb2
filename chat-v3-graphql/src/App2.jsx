...
import { client, gql, useQuery } from "./config/graphql.config.js";
...
function App2() {
  ...
  // const data = useQuery(GET_USERS).data;
  const [jsonChatMessages, setJsonChatMessages] = useState(chatMessages || JSON_CHAT_MESSAGES);
  const [jsonChatList, setJsonChatList] = useState(chatList || JSON_CHAT_LIST);
    ...
    const fetchUsers = async () => {
      ...
      if (result) {
        ...
        const users = result.users.reduce((acc, user) => {
          ...
          if (user_copy.title === "Cristian") {
            user_copy.avatar = Avatar;
          }
          acc[user.title] = user_copy;
          return acc;
        }, {});
          ...
          <div className="flex flex-row justify-between bg-[#F0F2F5] dark:bg-[#202C33] px-4 py-2 h-[66.44px]">
            <button className="flex flex-row gap-x-6 items-center ">
              <img
                ...
                src={chatUsers ? chatUsers[chatSelected].avatar : ""}
